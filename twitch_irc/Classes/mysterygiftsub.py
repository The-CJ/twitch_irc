...
from ..Utils.regex import (
	...
	ReMsgParamMassGiftCount, ReMod, ReMsgParamSubPlan,
	ReMsgParamSenderCount, ReRoomID, ReSubscriber,
	ReSystemMsg, ReTMISendTS, ReTurbo,
	ReUserID, ReUserType, ReRoomName
)

class MysteryGiftSub(object):
	"""
	This Class represents a submysterygift, u may also call this a: "Random Mass Sub" or so
    (in this event is not tracked who got the subs, just the one who buyed the subs)
	"""
	def __repr__(self):
		return f"<{self.__class__.__name__} channel='{self.room_name}' gifter='{self.user_name}' amount='{self.mass_gift_count}'>"
